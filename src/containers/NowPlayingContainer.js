...
class NowPlayingContainer extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    this.setState({
      nowPlayingList: nextProps.nowPlayingList,
      genres: nextProps.genreData
    });
  }
  ...
  prevPage() {
    ...
    this.props.getNowPlayingList(this.state.page - 1).then(() => {
  ...
  handleChange(val) {
    ...
    this.props.searchByGenre(val).then(() => {
...
function mapDispatchToProps(dispatch) {
  return bindActionCreators(
    { getNowPlayingList, getGenreData, searchByGenre },
    dispatch
  );
}
